...
# ---------------- ADMIN PANEL (WITH TABS: CHAT ANALYTICS + USER QUERY + QUERY ANALYTICS) ----------------
                    ...
                    with res_right:
                        ...
                        if not entities:
                            ...
                            for e in entities:
                                ...
                                val = e.get("value")
                                icon = "💲" if "AMOUNT" in ent_name.upper() else "🏦" if "ACCOUNT" in ent_name.upper() else "🆔"
                                st.markdown(
                                    f"""
                                    <div class="entity-card">
                                      <div class="entity-icon">{icon}</div>
                                      <div>
                                        <div style="font-weight:700;color:#115e59;">{ent_name.replace('_',' ').title()}</div>
                                        <div class="small-muted">{val}</div>
                                      </div>
                                    </div>
                                    """,
                                    unsafe_allow_html=True
    ...
    with tab3:
        ...
        nlu_low_conf = pd.read_sql(
            "SELECT COUNT(*) FROM nlu_history WHERE confidence < 0.7", 
            conn
        ).iloc[0, 0]
        ...
        if not recent_nlu.empty:
            st.dataframe(recent_nlu, width='stretch', hide_index=True, height=400)
        else:
            st.info("📊 No queries yet. Use User Query tab to analyze!")
